...
function onEdit(e) {
  ...
  const isOneCell = (cols - col) + (rows - row) === 0; 
...
root {
  ...
  --bg: ${config.fontColorLight.asRgbColor().asHexString()};
}`;
...
function createTemplateFromFile(filename, title) {
  return HtmlService.createTemplateFromFile(filename)
    .evaluate()
    .setTitle(title);
}

function createTemplate(filename, title) {
  return HtmlService.createTemplate(filename)
    .evaluate()
    .setTitle(title);
}
...
function toast(message, title, time) {
  SpreadsheetApp.getActiveSpreadsheet().toast(message, title, time);
};
...
function manualAddPilot() {
  ...
  const sidebar = createTemplate(source, title)
...
function helpDialog() {
  ...
  <head>
    ...
    <base target="top">
    <title>🦀 Wormhole Loot-O-Meter Help</title>
...
function clearInputs() {
  ...
  if (response !== ui.Button.YES) return actionCanceled();
